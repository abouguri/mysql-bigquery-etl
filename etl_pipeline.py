...
class ETLPipeline:
    ...
    Handles extraction, transformation, and loading of data, with metadata tracking and config validation.
    """
    ...
    def transform_data(self, df: pd.DataFrame, transformations: List[Any]) -> pd.DataFrame:
        ...
        Supports both string keys for built-in transforms and callables for custom logic.
        """
        try:
            if df.empty:
                return df
    ...
    def run_pipeline(self) -> bool:
        ...
        try:
            ...
            for table_config in self.config.etl_tables:
                ...
                self.logger.info(f"\n--- Processing table: {table_config['mysql_table']} ---")
                # Extract
                ...
                self.logger.info(f"Successfully processed {table_config['mysql_table']} in {elapsed:.2f}s")
