...
Current budget is **$0**. Cloud gates below are deferred. The active track is the [local MySQL-to-SQLite demo](local-demo.md), [verified process-crash recovery](evidence/local-recovery.md) and [completed real MySQL extraction benchmarks](../benchmarks/results/mysql-summary.md). All three have reproducible commands and recorded evidence. Next at $0: independent reproduction, a screen recording, and source-impact/destination-publication measurements.
