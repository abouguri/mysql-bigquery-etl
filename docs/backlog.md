...
Current budget is **$0**. Cloud gates below are deferred. The active track is the [local MySQL-to-SQLite demo](local-demo.md), [verified process-crash recovery](evidence/local-recovery.md) and forthcoming benchmarks that include real MySQL extraction.
